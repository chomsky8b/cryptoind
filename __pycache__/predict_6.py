...
class MyModeler:
  ...
  #       # Add the predicted and actual values
  #       'Predicted Values': list(self.predicted_values.values()),
  #       'Actual Values': list(self.actual_values.values())
  #   })
  ...
  def save_results(self, filename):
      results_list = []
      for d in self.netrets.keys():
          timestamps = self.predicted_values[d].index.astype(str)
          predicted_values = self.predicted_values[d].values.flatten().astype(str)
          actual_values = self.actual_values[d].values.flatten().astype(str)

          # Find the maximum length
          max_len = max(len(timestamps), len(predicted_values), len(actual_values))

          # Pad the shorter arrays with 'NaN'
          timestamps = timestamps.str.pad(max_len, side='right', fillchar='NaN')
          predicted_values = predicted_values.str.pad(max_len, side='right', fillchar='NaN')
          actual_values = actual_values.str.pad(max_len, side='right', fillchar='NaN')
...
mymod.save_results('results_analyze.xlsx')
